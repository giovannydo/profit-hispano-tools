...
/* Ocultar header completo - ya no necesitamos sidebar */
...
/* Expander (panel de parámetros) — destacado */
[data-testid="stExpander"] {
    background: linear-gradient(135deg, rgba(74,108,247,0.15), rgba(72,217,224,0.05)) !important;
    border: 1px solid rgba(72,217,224,0.4) !important;
    border-radius: 12px !important;
    margin-bottom: 8px !important;
    box-shadow: 0 0 20px rgba(72,217,224,0.08) !important;
}
...
# ══════════════════════════════════════════════════════════════════════
#  PLOTLY THEME — Leyenda DENTRO del gráfico
# ══════════════════════════════════════════════════════════════════════
PLOT = dict(
...
# Config para desactivar gestos de zoom problemáticos en móvil
PLOT_CONFIG = {
...
# ══════════════════════════════════════════════════════════════════════
#  PANEL DE PARÁMETROS (Expander — visible siempre, abierto por defecto)
# ══════════════════════════════════════════════════════════════════════
with st.expander("⚙  AJUSTA TUS DATOS — Capital, tasa, plazo y aportación mensual", expanded=True):
    ...
    with col4:
        aportacion = st.number_input(
            ...
            help="Cuánto agregas cada mes. Incluso $50 hace una gran diferencia a largo plazo."
        )

    r72 = regla_72(tasa)
    st.markdown(f"""
    <div style="background:rgba(74,108,247,0.08); border-left:3px solid #48D9E0;
                border-radius:6px; padding:8px 12px; margin-top:8px;
                font-size:12px; color:#7A94C8; line-height:1.6;">
        📐 <b style="color:#EEF2FF;">¿Sabías que?</b> Con una tasa del
        <b style="color:#48D9E0;">{tasa*100:.1f}%</b> anual, tu dinero se duplica cada
        <b style="color:#EEF2FF;">{r72:.1f} años</b> (Regla del 72).
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(
        x=[años], y=[val_final],
        mode="markers",
        marker=dict(color="#48D9E0", size=10, line=dict(color="#12082A", width=2)),
    ...
    fig1.update_layout(
        ...
        hovermode="x unified", height=340, **PLOT)
...
with tab2:
    ...
    with col_b:
        ...
        fig3.update_layout(
            ...
            font=dict(family='Sora, sans-serif', color='#7A94C8', size=11),
            margin=dict(l=6, r=6, t=28, b=6),
            legend=dict(font=dict(size=11, color='#EEF2FF')),
        )
